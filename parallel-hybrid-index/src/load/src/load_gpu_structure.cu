...
d_data loadGPUStructures(u_char *text, u_int size, 
	...
	u_long* d_sgcpft;
	cudaMalloc((void**) &d_sgcpft, lenArray*sizeof(u_long));
	cudaMemcpy(d_sgcpft, SGCPFT, lenArray*sizeof(u_long), cudaMemcpyHostToDevice);
	...
	u_long* d_sgcpt;
	cudaMalloc((void**) &d_sgcpt, lenArray*sizeof(u_long));
	cudaMemcpy(d_sgcpt, SGCPT, lenArray*sizeof(u_long), cudaMemcpyHostToDevice);
	...
	u_long* d_phraft;
	cudaMalloc((void**) &d_phraft, lenArray*sizeof(u_long));
	cudaMemcpy(d_phraft, PhraFT, lenArray*sizeof(u_long), cudaMemcpyHostToDevice);


	lenArray = z*lgPT / (8*sizeof(u_long));
